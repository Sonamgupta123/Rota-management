...
const WeeklyMedicationAudit = ({ selectedAudit, submitAuditResult, setSelectedAudit, isEditMode }) => {
  ...
  useEffect(() => {
    if (selectedAudit.status === 'Completed') {
      if (selectedAudit.details) {
        ...
        setManagerComments(details.signatures?.managerComments || 'Standard weekly review completed.');
        ...
        const yesCount = Math.round((22 * scoreVal) / 100);

        weeklyMedicationConfig.questions.forEach((q, idx) => {
          if (idx < yesCount) {
            tempAnswers[q.id] = 'YES';
            tempComments[q.id] = '';
          } else {
            tempAnswers[q.id] = 'NO';
            tempComments[q.id] = 'Weekly record verification gap.';
          }
        });
        ...
        const noCount = 22 - yesCount;
        const mockActions = [];
        for (let i = 0; i < noCount; i++) {
          const q = weeklyMedicationConfig.questions[yesCount + i];
          if (q) {
            mockActions.push({
              section: q.section,
              problem: `Weekly gap: ${q.question.substring(0, 50)}...`,
              actions: `Rectify and verify weekly medication check for: ${q.question.substring(0, 40)}`,
        ...
        setManagerComments(scoreVal < 90 ? 'Action items logged for priority compliance check.' : 'Weekly standards checked and verified.');
      ...
      weeklyMedicationConfig.questions.forEach(q => {
  ...
  // Dynamic calculations based on formula: YES and N/A = 1, NO = 0. Divide by 22 (or 19, let's divide by 22).
  const getCalculatedScore = () => {
    ...
    return Math.round((scoreCount / 22) * 100);
  };
  ...
  return (
    <div className="bg-[#fcfdfd] dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 sm:p-7 shadow-lg space-y-6 max-w-[1450px] mx-auto print:border-none print:shadow-none print:bg-white print:p-0">
      ...
      {/* Header Panel with double border style */}
      ...
      <div className="grid gap-6 md:grid-cols-2 select-none">
        ...
        <div className="border border-[#70ad47] dark:border-[#548235] rounded-lg p-5 bg-white dark:bg-slate-900 space-y-3 shadow-2xs">
          ...
          <p className="text-[10.5px] text-slate-500 dark:text-slate-400 leading-relaxed font-semibold italic">
            Calculate the number of Yes and N/A answers and divide by 19 then multiply by 100 – this will give you an overall % of compliance.
          </p>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse border-slate-400 min-w-[1100px] text-xs">
          <thead>
            <tr className="bg-[#70ad47] text-white font-extrabold uppercase border-b border-black">
              <td colSpan="5" className="p-3 text-center text-xs font-black tracking-wide bg-[#548235] border-b border-black">
                Audit to be completed every week for ALL residents per Unit / Floor and findings discussed during the Weekly Clinical Risk Meeting. All residents will be checked over a 4 week period
              </td>
            </tr>
            <tr className="bg-[#70ad47] text-white font-extrabold uppercase border-b-2 border-black dark:bg-[#548235] dark:border-slate-800 text-[11px]">
              <th className="p-3 border-r border-black dark:border-slate-800 w-[45%] align-middle text-left font-extrabold">Standard</th>
              <th className="p-3 border-r border-black dark:border-slate-800 w-[6%] align-middle text-center font-extrabold">Y</th>
              <th className="p-3 border-r border-black dark:border-slate-800 w-[6%] align-middle text-center font-extrabold">N</th>
              <th className="p-3 border-r border-black dark:border-slate-800 w-[6%] align-middle text-center font-extrabold">N/A</th>
              <th className="p-3 w-[37%] align-middle text-left font-extrabold">Initials of Residents checked: Comments</th>
            </tr>
          </thead>
            ...
            {Object.keys(sectionsMap).map((sectionName) => (
              <React.Fragment key={sectionName}>
                ...
                <tr className="bg-[#e2f0d9] border-y border-black dark:border-slate-800 dark:bg-slate-800 font-black text-[#385723] dark:text-[#a9d18e]">
                  <td colSpan="5" className="p-2.5 pl-3 text-xs font-black uppercase tracking-wider">
                    {sectionName}
                  </td>
                </tr>
                {sectionsMap[sectionName].map((q) => {
                  ...
                  return (
                    <tr key={q.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors border-b border-black dark:border-slate-800">
                      ...
                      {/* Initials of Residents checked: Comments */}
                      <td className="p-2 align-top">
                        <textarea
                          ...
                          placeholder="Initials and findings..."
                        />
                      </td>
                    </tr>
                  );
      ...
      <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
        ...
        <p className="text-[10.5px] text-slate-500 font-semibold leading-relaxed">
          If any of the weekly medication audit criteria above was marked as <strong className="text-rose-600 uppercase font-black">NO</strong>, a corrective action line must be generated below detailing the path to resolution.
        </p>
...
export default WeeklyMedicationAudit;
