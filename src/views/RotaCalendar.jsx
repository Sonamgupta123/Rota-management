    ...
    <div className="space-y-6 animate-fade-in p-1 max-w-[1400px] mx-auto">
              ...
              {DAYS.map(d => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 text-xs font-bold rounded-xl transition-all flex items-center justify-between
                    ${selectedDailyDay === d
                      ? 'bg-brand-50 text-brand-650 dark:bg-brand-950/20 dark:text-brand-400 border border-brand-200 dark:border-brand-900/50'
      ...
      {activeTab === 'monthly' && (
        <div className="glass-card rounded-2xl p-5">
          ...
          <div className="grid grid-cols-7 gap-1 bg-slate-100 dark:bg-slate-850 p-1.5 rounded-xl text-center text-xs font-bold text-slate-500 mb-2">
          ...
          <div className="grid grid-cols-7 gap-2">
            {Array.from({ length: totalDaysInMonth }).map((_, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className={`min-h-[90px] p-2 border rounded-xl flex flex-col justify-between transition-all bg-white dark:bg-slate-900/60
                    ${hasShifts ? 'border-brand-100 dark:border-brand-900/30' : 'border-slate-100 dark:border-slate-800'}
                  `}
                >
                  <div className="flex justify-between items-start">
                    <span className="font-bold text-slate-700 dark:text-slate-300 text-xs">{dateNum}</span>
                    {hasShifts && (
                      <span className="px-1 py-0.2 rounded bg-indigo-50 text-indigo-650 text-[8px] font-extrabold border border-indigo-100 dark:bg-indigo-950/20 dark:text-indigo-400 dark:border-indigo-900/40">
                        {dayShifts.length} Sh
                      </span>
                    )}
                  </div>

                  <div className="space-y-0.5 max-h-12 overflow-y-auto mt-1 custom-scrollbar">
                    {dayShifts.slice(0, 2).map(sh => {
                      const emp = employees.find(e => e.id === sh.employeeId);
                      return (
                        <div key={sh.id} className="text-[8px] font-semibold text-slate-500 dark:text-slate-400 truncate">
                          • {emp ? emp.name.split(' ')[0] : 'Staff'} ({sh.type.split('–')[0]})
                        </div>
                      );
                    })}
                    {dayShifts.length > 2 && (
                      <div className="text-[7px] font-bold text-brand-600 dark:text-brand-400">
